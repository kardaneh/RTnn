#!/bin/bash
echo "Time = 20250626_100849"
echo "Running training with model: LSTM_64_2, loss: nmse"

python src/main_lsm.py \
    --root_dir "./" \
    --main_folder "Debug__LSTM_64_2_bs_16_tb_1_nw_1_ep_1" \
    --sub_folder "lr_0d001_beta_0d001_lf_nmse_date_20250626_100849" \
    --train_data_files "/leonardo_work/EUHPC_D17_070/Data_LSM_1990-2000/" \
    --test_data_files "/leonardo_work/EUHPC_D17_070/Data_LSM_1990-2000/" \
    --train_year "1998" \
    --test_year "1999" \
    --prefix "lr_0d001_beta_0d001_lf_nmse_date_20250626_100849" \
    --dataset_type "LSM" \
    --loss_type "nmse" \
    --learning_rate "0.001" \
    --beta "0.001" \
    --batch_size "16" \
    --tbatch "24" \
    --model_name "LSTM_64_2" \
    --num_workers "1" \
    --num_epochs "1" \
    --save_mode "True" \
    --save_checkpoint_name "model" \
    --save_per_samples 10000 \
    --load_model "False"
