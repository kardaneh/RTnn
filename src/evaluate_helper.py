...
def MSELoss_valid(predict, true):
    ...
    if valid_length == 0:
        ...
        return valid_length, torch.mean(
            torch.square(predict[valid_index] - true[valid_index])
        )
...
def unnormalized_mpas(predicts, targets, norm_mapping, index_mapping):
    ...
    for index, value in index_mapping.items():
        predicts_unnorm[:, index, :] = (
            predicts[:, index, :] * norm_mapping[index_mapping[index]]["scale"]
            + norm_mapping[index_mapping[index]]["mean"]
        )

        targets_unnorm[:, index, :] = (
            targets[:, index, :] * norm_mapping[index_mapping[index]]["scale"]
            + norm_mapping[index_mapping[index]]["mean"]
        )
...
def get_heat_rate(predicts, targets, pressure):
    swhr_predict = calculate_hr(predicts[:, 0:1, :], predicts[:, 1:2, :], pressure)
    swhr_target = calculate_hr(targets[:, 0:1, :], targets[:, 1:2, :], pressure)
    # the last of the short wave do not need to calculate
    swhr_predict[:, :, -1] = 0.0
    swhr_target[:, :, -1] = 0.0

    lwhr_predict = calculate_hr(predicts[:, 2:3, :], predicts[:, 3:4, :], pressure)
    lwhr_target = calculate_hr(targets[:, 2:3, :], targets[:, 3:4, :], pressure)
...
def check_accuracy_evaluate(
    ...
    with torch.no_grad():
        ...
        for batch_idx, (feature, targets, auxis) in enumerate(loader):
            ...
            if (
                ...
                or (args.dataset_type == "Small")
                or (args.dataset_type == "FullYear")
                or (args.dataset_type == "WRF")
            ):
                ...
                auxis = auxis.reshape(
                    inner_batch_size, auxis_shape[2], auxis_shape[3]
                ).to(device=device)
            else:
            ...
            if args.dataset_type == "CliMart":
                ...
                predicts_unnorm, targets_unnorm = unnormalized_climart(
                    predicts, targets, target_mean, target_std
                )
            ...
            valid_length, valid_value = MSELoss_all(
                predicts_unnorm[:, 0:2, 0], targets_unnorm[:, 0:2, 0]
            )
            sw_flux_bottom_rmse.update(valid_value.item(), valid_length)

            valid_length, valid_value = MSELoss_all(
                predicts_unnorm[:, 0:2, -1], targets_unnorm[:, 0:2, -1]
            )
            ...
            valid_length, valid_value = MSELoss_all(
                predicts_unnorm[:, 2:4, 0], targets_unnorm[:, 2:4, 0]
            )
            ...
            if if_plot:
                if batch_idx < 50:
                    ...
                    file_name_RTM = os.path.join(
                        ...
                        args.main_folder,
                        args.sub_folder,
                        "Flux" + str(batch_idx) + ".png",
                    )
                    plot_RTM(
                        predicts_unnorm, targets_unnorm, file_name_RTM, sample_index=0
                    )
    ...
    """
    return (
        sw_flux_rmse.getsqrtmean(),
        lw_flux_rmse.getsqrtmean(),
        sw_hr_rmse.getsqrtmean(),
        lw_hr_rmse.getsqrtmean(),
        sw_hr_mae.getmean(),
        lw_hr_mae.getmean(),
        sw_flux_bottom_rmse.getsqrtmean(),
        lw_flux_bottom_rmse.getsqrtmean(),
        sw_flux_top_rmse.getsqrtmean(),
        sw_flux_mbe.getmean(),
        sw_flux_bottom_mbe.getmean(),
        sw_flux_top_mbe.getmean(),
    )


def check_accuracy(
    loader, model, norm_mapping, index_mapping, device, args, target_norm_info=None
):
    """
    外面包了一层汇总的
    """
    (
        ...
        device,
        args,
        target_norm_info=target_norm_info,
    )
    return (
        [sw_flux_rmse, lw_flux_rmse, sw_hr_rmse, lw_hr_rmse, sw_hr_mae, lw_hr_mae],
        [
